...
namespace SwachhBharat.API.Bll.Repository.Repository
{
    public class Repository : IRepository
    {
        ...
        public SBUser CheckUserLogin(string userName, string password, string imi, int AppId, string EmpType)
        {
            ...
            if (EmpType == "DSI")
            {
                user = CheckUserLoginForDSI(userName, password, imi, AppId, EmpType);
            }
        ...
        public SBUser CheckUserLoginForDSI(string userName, string password, string imi, int AppId, string EmpType)
        {
            SBUser user = new SBUser();
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                var objmain = dbMain.AppDetails.Where(x => x.AppId == AppId).FirstOrDefault();
                var AppDetailURL = objmain.baseImageUrlCMS + objmain.basePath + objmain.UserProfile + "/";
                var obj = db.UserMasters.Where(c => c.userLoginId == userName & c.isActive == true & c.EmployeeType == "DSI").FirstOrDefault();
        ...
        public Result SaveUserAttendence(SBUserAttendence obj, int AppId, int type, string batteryStatus)
        {
            ...
            if (obj.EmpType == "DSI")
            {
                result = SaveUserAttendenceForDSI(obj, AppId, type, batteryStatus);
            }
            return result;
        ...
        public Result SaveUserAttendenceForDSI(SBUserAttendence obj, int AppId, int type, string batteryStatus)
        {
            Result result = new Result();
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                var user = db.UserMasters.Where(c => c.userId == obj.userId && c.EmployeeType == "DSI").FirstOrDefault();
                ...
                if (type == 0)
                {
                    ...
                    {
                        //Daily_Attendance data = db.Daily_Attendance.Where(c => c.daDate == EntityFunctions.TruncateTime(obj.daDate) && c.userId == obj.userId && (c.endTime == null || c.endTime == "")).FirstOrDefault();
                        Daily_Attendance data = db.Daily_Attendance.Where(c => c.userId == obj.userId && (c.endTime == null || c.endTime == "") && c.EmployeeType == "DSI").FirstOrDefault();
                        if (data != null)
                        {
                            data.endTime = obj.startTime;
                            data.daEndDate = obj.daDate;
                ...
                else
                {
                    ...
                    try
                    {
                        Daily_Attendance objdata = db.Daily_Attendance.Where(c => c.daDate == EntityFunctions.TruncateTime(obj.daDate) && c.userId == obj.userId && (c.endTime == "" || c.endTime == null) && c.EmployeeType == "DSI").FirstOrDefault();
                        if (objdata != null)
                        {
                        ...
                        {
                            Daily_Attendance objdata2 = db.Daily_Attendance.Where(c => c.userId == obj.userId && (c.endTime == "" || c.endTime == null) && c.EmployeeType == "DSI").OrderByDescending(c => c.daID).FirstOrDefault();
                            objdata2.userId = obj.userId;
                            objdata2.daEndDate = DateTime.Now;
                            objdata2.endLat = obj.endLat;
                            objdata2.endLong = obj.endLong;
        ...
        public List<SyncResult1> SaveUserAttendenceOffline(List<SBUserAttendence> obj, int AppId, string cdate, string EmpType)
        {
            ...
            if (EmpType == "CT")
            {
                result = SaveUserAttendenceOfflineForCT(obj, AppId, cdate, EmpType);
            }

            if (EmpType == "DSI")
            {
                result = SaveUserAttendenceOfflineForDSI(obj, AppId, cdate, EmpType);
            }
        ...
        public List<SyncResult1> SaveUserAttendenceOfflineForCT(List<SBUserAttendence> obj, int AppId, string cdate, string EmpType)
        {
            List<SyncResult1> result = new List<SyncResult1>();
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
        ...
        public List<SyncResult1> SaveUserAttendenceOfflineForDSI(List<SBUserAttendence> obj, int AppId, string cdate, string EmpType)
        {
            List<SyncResult1> result = new List<SyncResult1>();
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                ...
                foreach (var x in obj)
                {
                    ...
                    DateTime Datee = Convert.ToDateTime(cdate);
                    var IsSameRecordLocation = db.Locations.Where(c => c.userId == x.userId && c.datetime == Datee && c.type == null && c.EmployeeType == "DSI").FirstOrDefault();
                    try
                    {
                        bool _IsInSync = false, _IsOutSync = false;
                        var user = db.UserMasters.Where(c => c.userId == x.userId && c.EmployeeType == "DSI").FirstOrDefault();
                        ...
                        if (user.isActive == true)
                        {
                            ...
                            var IsSameRecord = db.Daily_Attendance.Where(
                                   ...
                                   c.endTime == x.endTime &&
                                   c.daDate == EntityFunctions.TruncateTime(x.daDate) &&
                                   c.daEndDate == EntityFunctions.TruncateTime(x.daEndDate) && c.EmployeeType == "DSI"
                                 ).FirstOrDefault();

                            if (IsSameRecord == null)
                            {

                                var objdata = db.Daily_Attendance.Where(c => c.daDate == EntityFunctions.TruncateTime(x.daDate) && c.userId == x.userId && (c.endTime == "" || c.endTime == null) && c.EmployeeType == "DSI").FirstOrDefault();
                                if (objdata != null && x.endTime == null)
                                {
                                    objdata.endTime = x.startTime;
                                    objdata.daEndDate = x.daDate;
                                ...
                                {
                                    var OutTime = db.Daily_Attendance.Where(a => a.userId == x.userId && a.startTime == x.startTime && a.daDate == x.daDate && a.EmployeeType == "DSI").OrderByDescending(m => m.daID).FirstOrDefault();
        ...
        private CollectionSyncResult SaveCommercialCollectionSync(SBGarbageCollectionView obj, int AppId, int type)
        {
            int locType = 0;
            string mes = string.Empty;
            CollectionSyncResult result = new CollectionSyncResult();
            ...
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                ...
                var house = db.CommercialMasters.Where(c => c.ReferanceId == obj.CommercialId).FirstOrDefault();
                bool IsExist = false;
                DateTime Dateeee = Convert.ToDateTime(obj.gcDate);
                DateTime startDateTime = new DateTime(Dateeee.Year, Dateeee.Month, Dateeee.Day, 00, 00, 00, 000);
                DateTime endDateTime = new DateTime(Dateeee.Year, Dateeee.Month, Dateeee.Day, 23, 59, 59, 999);
                var IsSameHouseRecord = db.GarbageCollectionDetails.Where(c => c.userId == obj.userId && c.commercialId == house.commercialId && c.gcDate == Dateeee).FirstOrDefault();
                    ...
                    if (IsSameHouseRecord == null)
                    {
                        ...
                        try
                        {
                            ...
                            if (obj.CommercialId != null && obj.CommercialId != "")
                            {
                                ...
                                {
                                    locType = 1;
                                    objdata.commercialId = house.commercialId;
                                    name = house.commercialOwner;
                                    nameMar = checkNull(house.commercialOwnerMar);
                                    addre = checkNull(house.commercialAddress);
                                ...
                                {
                                    result.ID = obj.OfflineID;
                                    result.message = "Invalid Commercial Id"; result.messageMar = "अवैध व्यावसायिक आयडी";
                                    result.status = "error";
                                    return result;
                                }
                            ...
                            if (IsExist == true)
                            {
                                ...
                                var gcd = db.GarbageCollectionDetails.Where(c => c.commercialId == house.commercialId && c.userId == obj.userId && EntityFunctions.TruncateTime(c.gcDate) == EntityFunctions.TruncateTime(Dateeee)).FirstOrDefault();
                                if (gcd == null)
                                {
                                    result.ID = obj.OfflineID;
                                    result.message = "This Commercial Id already scanned."; result.messageMar = "हे व्यावसायिक आयडी आधीच स्कॅन केले आहे.";
                                    result.status = "error";
                                    return result;
                                }
                                if (gcd != null)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(obj.CommercialId))
                                    {
                                        loc.ReferanceID = obj.CommercialId;
                                    }
                                    loc.CreatedDate = DateTime.Now;
                            ...
                            else
                            {
                                if (house != null)
                                {
                                    if (house.commercialLat == null && house.commercialLong == null)
                                    {
                                        house.commercialLat = obj.Lat;
                                        house.commercialLong = obj.Long;
                                    }
                                }
        ...
        private CollectionSyncResult SaveCommercialCDCollectionSync(SBGarbageCollectionView obj, int AppId, int type)
        {
            int locType = 0;
            string mes = string.Empty;
            CollectionSyncResult result = new CollectionSyncResult();
                    ...
                    if (IsSameHouseRecord == null)
                    {
                        ...
                        try
                        {
                            ...
                            if (obj.CommercialId != null && obj.CommercialId != "")
                            {
                                ...
                                {
                                    result.ID = obj.OfflineID;
                                    result.message = "Invalid Commercial Costruction & Demolition Id"; result.messageMar = "अवैध व्यावसायिक बांधकाम आणि विध्वंस आयडी";
                                    result.status = "error";
                                    return result;
                                }
        ...
        private CollectionSyncResult SaveCommercialHWCollectionSync(SBGarbageCollectionView obj, int AppId, int type)
        {
            int locType = 0;
            string mes = string.Empty;
            CollectionSyncResult result = new CollectionSyncResult();
                    ...
                    if (IsSameHouseRecord == null)
                    {
                        ...
                        try
                        {
                            ...
                            if (obj.CommercialId != null && obj.CommercialId != "")
                            {
                                ...
                                {
                                    result.ID = obj.OfflineID;
                                    result.message = "Invalid Commercial Horticulture Id"; result.messageMar = "अवैध व्यावसायिक फलोत्पादन आयडी";
                                    result.status = "error";
                                    return result;
                                }
        ...
        private CollectionSyncResult SaveDSICollectionSync(SBGarbageCollectionView obj, int AppId, int type)
        {
            int locType = 0;
            string mes = string.Empty;
            CollectionSyncResult result = new CollectionSyncResult();
            ...
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                ...
                { 
                            ...
                            if(obj.dsiScanId!=null || obj.dsiScanId=="")
                    { 
                            objdata.gcType = obj.gcType;
                            objdata.gpBeforImage = obj.gpBeforImage;
                            objdata.gpAfterImage = obj.gpAfterImage;
                            objdata.note = checkNull(obj.note);
                            objdata.garbageType = null;
                            objdata.Wet = checkIntNull(obj.Wet.ToString());
                            objdata.Dry = checkIntNull(obj.Dry.ToString());
                            objdata.vehicleNumber = checkNull(obj.vehicleNumber);
                            loc.Distnace = obj.Distance; // Convert.ToDecimal(distCount);
                            ...
                            objdata.userId = obj.userId;
                            objdata.LOS = obj.LevelOS;
                            objdata.EmployeeType = obj.EmpType;
                            objdata.locAddresss = addre;
                            objdata.CreatedDate = DateTime.Now;
                            objdata.WasteType = obj.wastetype;
                            objdata.CGarbageType = obj.garbageTypeC;
                            var Pn = db.UserMasters.Where(x => x.userId == obj.userId).FirstOrDefault();
                            objdata.PrabhagId = Pn.PrabhagId;
                        if (!string.IsNullOrEmpty(obj.dsiScanId))
                        {
                            objdata.dsiScanId = obj.dsiScanId;
                        }
                        db.GarbageCollectionDetails.Add(objdata);
                            ...
                            loc.Distnace = obj.Distance;
                            loc.IsOffline = obj.IsOffline;
                            loc.EmployeeType = obj.EmpType;
                        if (!string.IsNullOrEmpty(obj.dsiScanId))
                            {
                                loc.ReferanceID = obj.dsiScanId;
                            }
                            loc.CreatedDate = DateTime.Now;
        ...
        private CollectionSyncResult GetHouseDetails(SBGarbageCollectionView obj, int AppId, int type)
        {
            ...
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                ...
                try
                {
                    ...
                    result.ID = obj.OfflineID;
                    result.status = "success";
                    result.message = " Get House Detail  successfully";
                    result.messageMar = "सबमिट यशस्वी";
                    result.messageMar = "सबमिट यशस्वी";
                    result.CType = dydetails.CType;
            ...
            if (obj.IsLocation == false && house != null && appdetails.IsScanNear == true)
            {
                switch (obj.gcType)
                {
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        result = GetHouseDetails(obj, AppId, type);
                        break;

                    case 9:
                        result = SaveCommercialCollectionSync(obj, AppId, type);
                        break;
                    ...
                    case 12:
                        result = SaveCommercialCDCollectionSync(obj, AppId, type);
                        break;

                    case 13:
                        result = SaveCommercialHWCollectionSync(obj, AppId, type);
                        break;
                    case 14:
                        result = SaveDSICollectionSync(obj, AppId, type);
                        break;
        ...
        public List<SBWorkDetails> GetUserWork(int userId, int year, int month, int appId, string EmpType)
        {
            ...
            if (EmpType == "DSI")
            {
                obj = GetUserWorkForDSI(userId, year, month, appId);
            }
            return obj;
        }
        ...
        public List<SBWorkDetails> GetUserWorkForDSI(int userId, int year, int month, int appId)
        {
            List<SBWorkDetails> obj = new List<SBWorkDetails>();
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(appId))
            {
                var data = db.GetAttendenceDetailsDSI(userId, year, month).ToList();
                foreach (var x in data)
                {
                    ...
                    {
                        date = Convert.ToDateTime(x.day).ToString("MM-dd-yyy"),
                        DSICollection = checkIntNull(x.DSICollection.ToString()),
                    });
                }
                ...
                foreach (var x in data)
                {
                    ...
                    if (x.gcType == 14)
                    {
                        try
                        {
                            ...
                            if (languageId == 1)
                            {
                                Name = checkNull(x.dsiScanId);
                                area = db.TeritoryMasters.Where(c => c.Id == x.AreaId).FirstOrDefault().Area;
                            }
                            else
                            {
                                Name = checkNull(x.dsiScanId);
                                area = db.TeritoryMasters.Where(c => c.Id == x.AreaId).FirstOrDefault().AreaMar;
                            }
        ...
        public List<AHouseGarbageCollectionVM> GetHouseGarbageCollectionData(int appId, int userId, DateTime fdate, DateTime tdate, int gcType, int Offset, int Fetch_Next, string SearchString)
        {
            ...
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(appId))
            {
                var data = db.SP_GarbageCollection(appId, userId, fdate, tdate, null, null, null, null, null).Select(x => new AHouseGarbageCollectionVM
                {
                    Id = x.gcId,
                    userId = x.userId,
                    houseId = x.houseId,
                ...
                try
                {
                    ...
                    if (gcType == 11)
                    {
                        ...
                        if (dump != null)
                        {
                            ...
                            {
                                dump.swmLong = obj.Long;
                            }

                            dump.modified = DateTime.Now;
                        ...
                        {
                            result.status = "error";
                            result.message = "Invalid CTPT ID";
                    ...
                    if (gcType == 10)
                    {
                        var dump = db.SauchalayAddresses.Where(x => x.ReferanceId == referanceid).FirstOrDefault();
                        if (dump != null)
                        {
                            if ((string.IsNullOrEmpty(obj.name)) == false)
                            {
                            ...
                            if ((string.IsNullOrEmpty(obj.namemar)) == false)
                            {
                                dump.Name = obj.namemar;
                            }
                            if ((string.IsNullOrEmpty(obj.Address)) == false)
                            {
                                dump.Address = obj.Address;
                            }
                            if ((string.IsNullOrEmpty(obj.Lat)) == false)
                            {
                                dump.Lat = obj.Lat;
            ...
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                try
                {
                    foreach (var item in obj)
                    {
                        ...
                        if (item.gcType == 11)
                        {
                            var dump = db.SWMMasters.Where(x => x.ReferanceId == item.ReferanceId).FirstOrDefault();
                            if (dump != null)
                            {

                                if (!string.IsNullOrEmpty(item.name))
                                {
                                    dump.swmName = item.name;
                                }
                                if (!string.IsNullOrEmpty(item.namemar))
                                {
                                    dump.swmName = item.namemar;
                                }
                                if (!string.IsNullOrEmpty(item.Address))
                                {
                                    dump.swmAddress = item.Address;
                                }
                                if (!string.IsNullOrEmpty(item.Lat))
                                {
                                    dump.swmLat = item.Lat;
                                }
                                if (!string.IsNullOrEmpty(item.Long))
                                {
                                    dump.swmLong = item.Long;
                                }

                                dump.modified = item.date; //DateTime.Now;
                                ...
                                {
                                    dump.swmType = item.CType;
                                }
                                if ((string.IsNullOrEmpty(item.QRCodeImage)) == false)
                                {
                        ...
                        if (item.gcType == 10)
                        {
                            var dump = db.SauchalayAddresses.Where(x => x.ReferanceId == item.ReferanceId).FirstOrDefault();
                            if (dump != null)
                            {
                                ...
                                if ((string.IsNullOrEmpty(item.QRCodeImage.ToString())) == false)
                                {
                                    dump.QRCodeImage = item.QRCodeImage;
                                }
                                if ((string.IsNullOrEmpty(item.QRCodeImage)) == false)
                ...
                {
                    {
                        ...
                        //    });
                        var data = db.VehicleListWardWise(wardId, VehicleTypeId).ToList();
                        foreach (var x in data)
                        {
